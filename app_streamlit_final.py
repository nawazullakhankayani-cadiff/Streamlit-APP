...
# ----------------- Train Model -----------------
    ...
    st.header("AQI Prediction using Multiple Linear Regression Algorithm")
    if df.empty:
        ...
        if st.button("Train Multiple Linear Regression Algorithm now"):
            try:
                ...
                # Use the trained model immediately
                st.session_state['loaded_pipeline_data'] = res['pipeline']
                st.session_state['just_trained_model_name'] = (
                    res['model_path'].split(os.path.sep)[-1]
                )
